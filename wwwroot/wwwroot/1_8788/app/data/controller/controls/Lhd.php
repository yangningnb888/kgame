...
class Lhd extends Controller
{
    ...
    {
        $_ret = Db::table('jh_game_config')->field('profit')->where('gtype', 6)->where('level', 5)->find();
        return json([
            'code' => 0,
            'msg' => '',
            'data' => $_ret['profit']
    ...
    public function upprofit()
    {
        ...
        if (isset($post['profit']) && is_numeric($post['profit'])) {
            Db::query('UPDATE jh_game_config SET profit=' . $post['profit'] . ' WHERE gtype=6 AND level=5');
        }
        $_ret = Db::table('jh_game_config')->field('profit')->where('gtype', 6)->where('level', 5)->find();
        return json([
            'code' => 0,
    ...
    {
        $index = [1 => '龙', 2 => '和', 3 => '虎'];
        $ret = Db::query('SELECT id,region FROM jh_duoren_control where gtype=6 and level=5');
    ...
    {
        $index = [1 => 'long', 2 => 'space', 3 => 'hu'];
        $ret = Db::query('SELECT playerbets,endtime FROM jh_player_bet where gtype=6 and level=5');
        $all = [];
        foreach ($ret as $key => $value) {
            $all = $value;
        }
