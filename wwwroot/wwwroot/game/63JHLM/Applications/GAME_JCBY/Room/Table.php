...
class Table
{
    ...
    public function __construct($msg)
    {
        ...
        Logic::SendAll('Msg_JCBY_RoomInfo', [
    ...
    public function CreateFishes()
    {
        $this->mTimer = Timer::add(ZHEN, function () {
            ...
            foreach ($this->robotList as $key => $value) {
                ...
                if ($this->zhenTimes - $value['lastzhen'] >= 10 && $value['endtime'] > $curtime && $value['status'] > LOCK && $value['lock'] != 0) {
                    ...
                    if (!empty($this->curFinshList[$value['lock']])) {
                        ...
                        $id = $this->Msg_JCBY_ActShoot($data);
                        if ($id) {
                            $this->Msg_JCBY_GetFish(
                                [
            ...
            if (!empty($news)) {
                Logic::SendAll('Msg_JCBY_CreateFish', ['finsh' => $send], $this->roomRule['rid']);
    ...
    public function CreateFishBoom()
    {
        ...
        Logic::SendAll('Msg_JCBY_FishTide', ['tide' => $this->fishboomId], $this->roomRule['rid']);
    ...
    public function All_RECV($message)
    {
        switch ($message['event']) {
            case 'Msg_JCBY_Out':
                $this->Msg_JCBY_Out($message);
                break;
            case 'Msg_JCBY_ActChange':
                $this->Msg_JCBY_ActChange($message);
                break;
            case 'Msg_JCBY_ActShoot':
                $this->Msg_JCBY_ActShoot($message);
                break;
            case 'Msg_JCBY_GetFish':
                $this->Msg_JCBY_GetFish($message);
                break;
            case 'Msg_JCBY_Locking':
                $this->Msg_JCBY_Locking($message);
                break;
            case 'Msg_JCBY_FishBattery':
                $this->Msg_JCBY_FishBattery($message);
                break;
            default:
            {
     ...
     * @param array
     */
    public function Msg_JCBY_ActChange($message)
    {
        $uid = $message['uid'];
     ...
     * @return int
     */
    public function Msg_JCBY_ActShoot($message)
    {
        $uid = $message['uid'];
     ...
     * @param array
     */
    public function Msg_JCBY_GetFish($message)
    {
        $uid = $message['uid'];
            ...
            if ($fish['type'] < 90) {
                ...
                if ($fish['type'] > 100) {
                    ...
                    if ($fish['type'] >= 300 && $fish['type'] < 400) {
                        $data['types'] = $couples = $this->Algorithm->GetLightFishes(300);
        ...
        if (!empty($data['fish'])) {
            ...
            Logic::SendAll('Msg_JCBY_GetFish', $data, $this->roomRule['rid']);
        }
    }
     ...
     * @param array
     */
    private function Msg_JCBY_Locking($message)
    {
        ...
        Logic::SendAll('Msg_JCBY_Locking', $data, $this->roomRule['rid']);
    }
     ...
     * @param array
     */
    private function Msg_JCBY_FishBattery($message)
    {
        $uid = $message['uid'];
    ...
    private function GetGolds($type, $double)
    {
        $ret = 0;
        //随机加倍鱼类
        $rands = [16 => [1, 10], 17 => [40, 300], 25 => [60, 120], 23 => [40, 180], 21 => [80, 150], 600 => [10, 10]];
        if ($type > 100) {
    ...
    private function GetPlayerAgent ()
    {
        ...
        if ($this->playerAgent != $cur) {
            Logic::SendAll('Msg_JCBY_PlayerAgent', $this->playerAgent, $this->roomRule['rid']);
        }
    }
     ...
     * @param array
     */
    public function Msg_JCBY_Out($message)
    {
        Logic::SendAll('Msg_JCBY_Out', [
    ...
    public function EnterRoom($message)
    {
        ...
        Logic::SendRight($uid, 'Msg_JCBY_RoomInfo', [
            'time_ms' => MyTools::GET_MS(),
            'stoptime' => $this->stopTime,
            'fishlist' => $this->curFinshList,
            'players' => $this->mUserList,
        ...
        Logic::SendAll('Msg_JCBY_PlayerAct', [
            'seat' => $this->mUserList[$uid]['seat'],
            'player' => $message,
