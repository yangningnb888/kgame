...
class StaticValue
{
    ...
    public static function GetStatus ()
    {
        ...
        $rand = rand(1, 100);
        if ($rand <= RANDSTAND) {
            $ret['nexttime'] = time() + rand(1, 5);
        } elseif ($rand <= RANDLOCK) {
            $ret['status'] = LOCK;
            $ret['nexttime'] = time() + rand(1, 3);
        } elseif ($rand <= RANDLOCKGET) {
            $ret['status'] = LOCKGET;
            $ret['nexttime'] = time() + rand(TIME_CHANGETURN[0], TIME_CHANGETURN[1]);
        } else {
            $ret['status'] = GETFISH;
    ...
    public static function LockFish ($fishlist)
    {
        ...
        foreach ($fishlist as $key => $value) {
            ...
            if ((empty($value['createtime']) || ($time - $value['createtime'] > 1000 && $value['endtime'] - $time > 3000)) && $value['type'] >= 18) {
                $list[$key] = $value;
            }
