...
class Table
{
    ...
    public function __construct($msg)
    {
        ...
        Logic::SendAll('Msg_DNTG_RoomInfo', [
    ...
    public function CreateFishes()
    {
        $this->mTimer = Timer::add(ZHEN, function () {
            ...
            foreach ($this->robotList as $key => $value) {
                if ($curtime - $value['endtime'] > 3) {
                    $this->Msg_DNTG_Out(['uid' => $key]);
                    continue;
            ...
            if (MyTools::GET_MS() - $this->lastFishBoom >= $this->roomRule['vals']['fishboom'] * 1000 && MyTools::GET_MS() - $this->stopTime >= 4000) {
                $this->CreateFishBoom();
                return;
            ...
            foreach ($news as $key => $value) {
                ...
                if (!empty($value['fishes'])) {
                    ...
                    foreach ($value['fishes'] as $key1 => $value1) {
                        $this->finshLife[$key][$key1] = $value1 == 2 ? 20 * StaticValue::$FishList[$type]['life'] : StaticValue::$FishList[$type]['life'];
                    }
                } else {
            ...
            if (!empty($news)) {
                Logic::SendAll('Msg_DNTG_CreateFish', ['finsh' => $send], $this->roomRule['rid']);
                $this->curFinshList += $news;
            }
            if (MyTools::GET_MS() - $this->stopTime >= 3000) {
                $this->stopTime = 0;
            }
            $this->RefreshFinsh();
        });
    ...
    public function CreateFishBoom()
    {
        ...
        Logic::SendAll('Msg_DNTG_FishTide', ['tide' => $this->fishboomId], $this->roomRule['rid']);
    ...
    public function All_RECV($message)
    {
        switch ($message['event']) {
            case 'Msg_DNTG_Out':
                $this->Msg_DNTG_Out($message);
                break;
            case 'Msg_DNTG_ActChange':
                $this->Msg_DNTG_ActChange($message);
                break;
            case 'Msg_DNTG_ActShoot':
                $this->Msg_DNTG_ActShoot($message);
                break;
            case 'Msg_DNTG_GetFish':
                $this->Msg_DNTG_GetFish($message);
                break;
            case 'Msg_DNTG_Locking':
                $this->Msg_DNTG_Locking($message);
                break;
            case 'Msg_DNTG_FishBattery':
                $this->Msg_DNTG_FishBattery($message);
                break;
            default:
            {
     ...
     * @param array
     */
    public function Msg_DNTG_ActChange($message)
    {
        $uid = $message['uid'];
     ...
     * @return int
     */
    public function Msg_DNTG_ActShoot($message)
    {
        $uid = $message['uid'];

        if (count($this->userBullet[$uid]) > 15 || (isset($this->robotList[$uid]) && count($this->userBullet[$uid]) > 5)) {
     ...
     * @param array
     */
    public function Msg_DNTG_GetFish($message)
    {
        $uid = $message['uid'];
        ...
        if ($life <= 0) {
            if ($fish['type'] < 90) {
                if (!empty($mindex)) {
                    //删除鱼阵中鱼
                    if ($this->curFinshList[$fishid]['fishes'][$mindex] == 2) {
                        foreach ($this->curFinshList[$fishid]['fishes'] as $key => $value) {
                            if ($value > 0) {
                                $this->curFinshList[$fishid]['fishes'][$key] = 0;
                                $data['fish'][$key] = $this->GetGolds($fish['type'], $bullet);
                            }
                        }
                    } else {
            ...
            } elseif ($fish['type'] >= 100 && $fish['type'] < 600) {
                //删除鱼
                $this->PushFishId($fishid);
                //特殊种类鱼加成
                if ($fish['type'] > 100) {
                    if ($fish['type'] != 300) {
                        $data['fish'][$fishid] = $this->GetGolds(10, $bullet);  //特殊炸弹得分 10倍
                    } else {
                        $data['fish'][$fishid] = $this->GetGolds($fish['type'] % 100, $bullet);
                    }
                    if ($fish['type'] >= 300 && $fish['type'] < 400) {
                        $data['types'] = $couples = [300];
                    } elseif ($fish['type'] < 600) {
                        $couples = $fish['couples'];
                    }

                    if ($fish['type'] > 200 && $fish['type'] < 300) {
                        $data['fish'][$fishid] = 0;
                        foreach ($fish['couples'] as $key => $value) {
                            $data['fish'][$fishid] += $this->GetGolds($value, $bullet);
                        }
                        $data['fish'] = $this->Get_Fishes($data['fish'], $couples, $bullet);
                    } else {
                        $data['fish'][$fishid] = $this->GetGolds($fish['type'] % 100, $bullet);
                        $data['fish'] = $this->Get_Fishes($data['fish'], $couples, $bullet);
                    }
                ...
                if ($fish['type'] == 92) {
                    ...
                    $time = MyTools::GET_MS();
                    foreach ($this->curFinshList as $key => $value) {
                        if (empty($value['fishes']) && $time - $value['createtime'] >= 600) {
                            $data['fish'][$key] = $this->GetGolds($value['type'], $bullet);
                            $this->PushFishId($key);
                        } elseif (!empty($value['fishes'])) {
                            foreach ($value['fishes'] as $key1 => $value1) {
                                if ($value1 > 0) {
                ...
                } elseif ($fish['type'] == 601 || $fish['type'] == 602) {
                    ...
                    foreach ($this->curFinshList as $key => $value) {
                        if (empty($value['fishes'])) {
                            ...
                            if ($distance < 200 && $fish['type'] == 601) {
                                $data['fish'][$key] = $this->GetGolds($value['type'], $bullet);
                                $this->PushFishId($key);
                            } elseif ($distance < 300 && $fish['type'] == 602) {
                                $data['fish'][$key] = $this->GetGolds($value['type'], $bullet);
                                $this->PushFishId($key);
                            }
                        } else {
        ...
        if (!empty($data['fish'])) {
            ...
            Logic::SendAll('Msg_DNTG_GetFish', $data, $this->roomRule['rid']);
        }
    }
     ...
     * @param array
     */
    private function Msg_DNTG_Locking($message)
    {
        ...
        Logic::SendAll('Msg_DNTG_Locking', $data, $this->roomRule['rid']);
    }
     ...
     * @param array
     */
    private function Msg_DNTG_FishBattery($message)
    {
        $uid = $message['uid'];
    ...
    private function GetGolds($type, $double)
    {
        $ret = 0;
        //随机加倍鱼类
        $rands = [16 => [40, 300], 17 => [200, 300], 18 => [80, 120], 23 => [40, 180], 21 => [80, 150], 700 => [10, 10], 25 => [80, 200]];
        if ($type > 100) {
    ...
    private function Get_Fishes($fishes, $types, $double)
    {
        if ($types[0] == 300) {
            //闪电鱼
            foreach ($this->curFinshList as $key => $value) {
                if ($value['type'] >= 300 && $value['type'] < 400) {
                    $type = $value['type'] % 100;
                    $fishes[$key] = StaticValue::$FishList[$type]['double'] * $double * $this->roomRule['doublescore'];
                    $this->PushFishId($key);
                }
            }
        } else {
        ...
        foreach ($this->robotList as $key => $value) {
            ...
            if ($value['status'] != STAND && $value['berrtytime'] <= $this->recordTime) {
                ...
                if ($value['level'] != $this->mUserList[$key]['level']) {
                    $this->Msg_DNTG_ActChange(
                        [
                            'event' => 'Msg_DNTG_ActChange',
                            'uid' => $key,
                            'data' => [
    ...
    private function GetPlayerAgent ()
    {
        ...
        if ($this->playerAgent != $cur) {
            Logic::SendAll('Msg_DNTG_PlayerAgent', $this->playerAgent, $this->roomRule['rid']);
        }
    }
    ...
    {
        if (($this->fishboomId && MyTools::GET_MS() - $this->lastFishBoom >= 8000) || $this->fishboomId == 0 && count($this->curFinshList) > 50) {
            if ($this->fishboomId) {
     ...
     * @param array
     */
    public function Msg_DNTG_Out($message)
    {
        Logic::SendAll('Msg_DNTG_Out', [
    ...
    public function EnterRoom($message)
    {
        ...
        Logic::SendRight($uid, 'Msg_DNTG_RoomInfo', [
            'time_ms' => MyTools::GET_MS(),
            'stoptime' => $this->stopTime,
            'fishlist' => $this->curFinshList,
            'players' => $this->mUserList,
        ...
        Logic::SendAll('Msg_DNTG_PlayerAct', [
            'seat' => $this->mUserList[$uid]['seat'],
            'player' => $message,
