...
class Events
{
    ...
    {
        MyTools::$LTYPE = LOGIC_HALL;
        MyTools::$businessWorkerID = "$businessWorker->lanIp:$businessWorker->name:$businessWorker->id";

        //实例化DB
        DBInstance::Init(Config::$dbConfig);
        //数据库心跳
        Timer::add(Config::$dbpingInterval, function () {
            DBInstance::$db->table('sysconfig')->select('val')->where(array('key' => 'GAME_LOGIN_PORT'))->one(true);
        });
    ...
    public static function onCentralMessage($connection, $message)
    {
        ...
        } elseif (is_callable(array('Hall', $callback))) {
            call_user_func_array(array('Hall', $callback), array($msg));
        } else {
            MyTools::msg('HALL onCentralMessage Unknown message : ' . $message);
        }
    }
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        if (is_callable(array('Common', $callback))) {
            ...
            MyTools::msg('HALL onMessage Unknown message : ' . base64_decode($message));
        }
    }
