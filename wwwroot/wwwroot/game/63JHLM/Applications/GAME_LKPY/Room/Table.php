...
class Table
{
    ...
    public function __construct($msg)
    {
        ...
        Logic::SendAll('Msg_LKPY_RoomInfo', [
    ...
    public function CreateFishes()
    {
        $this->mTimer = Timer::add(ZHEN, function () {
            ...
            foreach ($this->robotList as $key => $value) {
                if ($this->zhenTimes - $value['lastzhen'] < 8) {
                    continue;
                }
                if ($curtime - $value['endtime'] > 0) {
                    $this->Msg_LKPY_Out(['uid' => $key]);
                    continue;
            ...
            if (!empty($news)) {
                Logic::SendAll('Msg_LKPY_CreateFish', ['finsh' => $send], $this->roomRule['rid']);
    ...
    public function CreateFishBoom()
    {
        ...
        Logic::SendAll('Msg_LKPY_FishTide', ['tide' => $this->fishboomId], $this->roomRule['rid']);
    ...
    public function All_RECV($message)
    {
        switch ($message['event']) {
            case 'Msg_LKPY_Out':
                $this->Msg_LKPY_Out($message);
                break;
            case 'Msg_LKPY_ActChange':
                $this->Msg_LKPY_ActChange($message);
                break;
            case 'Msg_LKPY_ActShoot':
                $this->Msg_LKPY_ActShoot($message);
                break;
            case 'Msg_LKPY_GetFish':
                $this->Msg_LKPY_GetFish($message);
                break;
            case 'Msg_LKPY_Locking':
                $this->Msg_LKPY_Locking($message);
                break;
            case 'Msg_LKPY_FishBattery':
                $this->Msg_LKPY_FishBattery($message);
                break;
            default:
            {
     ...
     * @param array
     */
    public function Msg_LKPY_ActChange($message)
    {
        $uid = $message['uid'];
     ...
     * @return int
     */
    public function Msg_LKPY_ActShoot($message)
    {
        $uid = $message['uid'];
     ...
     * @param array
     */
    public function Msg_LKPY_GetFish($message)
    {
        $uid = $message['uid'];
        ...
        if ($this->fishboomId/* || isset($this->mRobot[$uid])*/) {
            $rand_get += BOOMRAND;
        }
        ...
        if (!empty($data['fish'])) {
            ...
            Logic::SendAll('Msg_LKPY_GetFish', $data, $this->roomRule['rid']);
        }
    }
     ...
     * @param array
     */
    private function Msg_LKPY_Locking($message)
    {
        ...
        Logic::SendAll('Msg_LKPY_Locking', $data, $this->roomRule['rid']);
    }
     ...
     * @param array
     */
    private function Msg_LKPY_FishBattery($message)
    {
        $uid = $message['uid'];
        ...
        foreach ($this->robotList as $key => $value) {
            ...
            if ($value['status'] != STAND && $value['berrtytime'] <= $this->recordTime) {
                ...
                if ($value['level'] != $this->mUserList[$key]['level']) {
                    $this->Msg_LKPY_ActChange(
                        [
                            'event' => 'Msg_LKPY_ActChange',
                            'uid' => $key,
                            'data' => [
    ...
    private function GetPlayerAgent ()
    {
        ...
        if ($this->playerAgent != $cur) {
            Logic::SendAll('Msg_LKPY_PlayerAgent', $this->playerAgent, $this->roomRule['rid']);
        }
    }
     ...
     * @param array
     */
    public function Msg_LKPY_Out($message)
    {
        Logic::SendAll('Msg_LKPY_Out', [
    ...
    public function EnterRoom($message)
    {
        ...
        Logic::SendRight($uid, 'Msg_LKPY_RoomInfo', [
            'time_ms' => MyTools::GET_MS(),
            'stoptime' => $this->stopTime,
            'fishlist' => $this->curFinshList,
            'players' => $this->mUserList,
        ...
        Logic::SendAll('Msg_LKPY_PlayerAct', [
            'seat' => $this->mUserList[$uid]['seat'],
            'player' => $message,
