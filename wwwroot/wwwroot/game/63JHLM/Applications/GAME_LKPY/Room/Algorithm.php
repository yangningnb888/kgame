...
class Algorithm
{
    ...
    public function __construct()
    {
        ...
        StaticValue::$FishList = DBInstance::GetFishList(GAME_LKPY);
    ...
    private function CreateBoom()
    {
        ...
        foreach (StaticValue::$fishBoom as $key => $value) {
            $list = [];
            foreach ($value['fish_list'] as $key1 => $value1) {
                /*var_dump($value1);
                break 2;*/
                $list[$key1] = [
                    'type' => $value1['type'],
                    'life' => $value1['type'] < 90 ? StaticValue::$FishList[$value1['type']]['life'] : StaticValue::$FishList[$value1['type'] % 10]['life'] + 50,
                ];
            }

            DBInstance::$db->table('fish_boom')->where(array('id' => $key))->update(['fish_list' => json_encode($list)]);
        }
    }
    ...
    public function GetNewFinsh($timer, $curlist)
    {
        ...
        $countspecil = [100 => 0, 200 => 0, 300 => 0];   //统计特殊鱼数量
        $fishes = [600 => ['rand' => 3, 'num' => 3], 500 => ['rand' => 5, 'num' => 7], 400 => ['rand' => 8, 'num' => 6]];  // 400直线队列  500群队列 600圆圈队列
        $ret = [];
        //统计各类鱼的当前数量
                        ...
                        if (mt_rand(1, 100) <= $value['new_rand']) {
                            ...
                            if ($key <= 10 && $key > 2) {
                                foreach ($fishes as $key1 => $value1) {
                                    if (($key <= 6 || $key1 == 600 && $key > 6 && !empty($circleWay)) && $rand < $value1['rand']) {
                                        $circlepoint = [];
                                        if ($key1 == 600 && !empty($circleWay)) {
                                            $way = 1000;
