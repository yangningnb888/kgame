    ...
    public function UpdateAll ($data = array())
    {
        ...
        foreach ($data['keys'] as $k => $v) {
            ...
            if ($strkey == '') {
                ...
                $strkey = $strkey . ' AND ' . $v;
                $strkeynum = $strkeynum . ' AND ' . $temp;
            }
        }
        $strkey = ' = CASE ' . $strkey;

        $keynum = count($data['keys']);
        foreach ($data['val'] as $key => $val) {
            $temp = '';
            foreach ($data['arr'] as $key1 => $val1) {
                $temp .= ' WHEN ' . $data['keys'][0] . ' = ' . $val1[0];
                for ($i = 1; $i < $keynum; $i++) {
                    $temp .= ' AND ' . $data['keys'][$i] . ' = ' . $val1[$i];
                }
                $temp .= ' THEN ' . $val1[$key + $keynum];
            }
            if ($key == 0) {
                $sql .= $val . ' = CASE ' . $temp . ' END';
            } else {
                $sql .= ', ' . $val . ' = CASE ' . $temp . ' END';
            }
        }

        $sql = "UPDATE " . $this->tablepre . $this->table ." SET $sql WHERE $strkeynum";
