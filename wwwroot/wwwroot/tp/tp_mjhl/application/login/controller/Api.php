...
class Api extends Controller
{
    ...
    public function uptel ()
    {
        ...
        $tels = Db::name('jh_user_register')->field('*')->where(['telephone' => $post['new_telephone']])->find();
        if ($tels) {
            $this->error('该手机号已经被使用');
            return;
        }
        /*$time = time();
        $info = Db::name('jh_tel_code')->field('code')->where('tel', $post['old_telephone'])->where('time', '<=', $time)->find();
        ...
        $time = time();
        $info = Db::name('jh_tel_code')->field('code')->where('tel', $post['new_telephone'])->where('time', '<=', $time)->find();
        if (!$info || $info['code'] != $post['new_code']) {
            $this->error('新验证码错误');
            return;
        }*/

        $user = Db::name('jh_user_register')->field('*')->where(['telephone' => $post['old_telephone']])->find();
        if ($user) {
            Db::name('jh_user_register')->where('uid', $user['uid'])->update(['telephone' => $post['new_telephone']]);
    ...
    public function feedback()
    {
        ...
        Db::name('jh_feedback')->insert(['uid' => $post['uid'], 'content' =>$post['content'], 'created' => $time]);
