...
class Slwh extends Controller
{
    ...
    {
        $datajson = array('status' => '1', 'msg' => "", 'data' => []);
        $player_bets = new Player_bet();
        $info = $player_bets->field('playerbets,endtime,last_type,last_win')->where('gtype', 4)->where('level', 5)->find();
        ...
        $res = Db::name('jh_game_config')->field('controls,profit')->where('level', 5)->where('gtype', 4)->find();
