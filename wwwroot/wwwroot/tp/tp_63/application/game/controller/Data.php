...
class Data extends Controller
{
    ...
    public function sysstop()
    {
        ...
        if (!$time1 || !$time2) {
            // 两个都传空，表示关闭维护（清零），不视为格式错误
            if ($post['start'] === '' && $post['end'] === '') {
                $sysconfig = new  Sysconfig();
                $sysconfig->where('key', 'SERVER_WHSTART')->update(['val' => '']);
                $sysconfig->where('key', 'SERVER_WHEND')->update(['val' => '']);
                $datajson['msg'] = '';
                $datajson['status'] = 1;
                $datajson['data'] = ['start' => '', 'end' => ''];
                return json($datajson);
            }
            return json($datajson);
        }
        ...
        // 避免后台"维护时间"误设全天窗口导致所有游戏无法进入。
        $isFullDay = ($post['start'] === '' || $post['end'] === '')
            || ($time1 <= strtotime('today 00:00:00') && $time2 >= strtotime('today 23:59:00'));
        if ($isFullDay) {
            $post['start'] = '';
            $post['end']  = '';
        }
